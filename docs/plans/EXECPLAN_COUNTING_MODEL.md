...
COMPLETE — closed out 2026-09-22. All five milestones passed their acceptance (see Progress and Outcomes & Retrospective). This document is immutable history from here on: corrections live in `docs/adr/` and `CLAUDE.md`, never as edits to this plan.
...
- [x] (2026-09-22) Close-out: retrospective written; ADR 0006 (committed score snapshot) and an addendum to ADR 0005 extracted; `CLAUDE.md` synced. Plan complete.
...
Achieved, against the three costs named in Purpose. The weekly update no longer needs a person for what the game routinely adds: a new trigger under a known phase, a new threshold condition, a new effect group or lesson kind are parsed, counted and published by rule, and what cannot be counted hides its card with a written reason instead of stopping the run (Milestone 1 and 4 acceptance: an invented threshold piece publishes, an invented unknown piece holds one card, a replay of the 2026-09-20 update passes the stability check with 2 new cards and 0 unexplained moves). Conditional effects are bounded counts under their occasion: stating 所持スキルカード20枚以上 for 3 of 8 SP lessons takes exactly 15 points from こんにゃくなきもだめし at 凸4 and moves no card without that condition. The panel went from a flat grid of 41 numbers to four sections of nested, bounded inputs that fold away what the cards in view do not use; the user judged it "fine for now". No published number moved in the migration: before the old model was deleted, all 204 cards × 4 route profiles × 凸0–4 scored identically under both, line by line (commit `3977df0`). The taxonomy, its hand-written extension rows, the classifier and the filter table are gone; the page's script is 56 KB gzipped, against 58 KB before the plan.

What remains, none of it this plan's. The weekly GitHub Action, the conditional pull request and the approval service belong to Milestone 5 of `docs/plans/EXECPLAN_SUPPORT_CARD_SCORE_TABLE.md`, which this plan unblocks (C9): the action runs `bun run generate`, `bun test` and `bun scripts/check-score-stability.ts`, publishes when all pass, and turns a non-empty `data/held.generated.ts` into the question for the user. The 初LEGEND profile is still the agent's estimate, including the upgrade effect-group default carried over by C13. No shipped profile lowers a condition yet (C7); that is the user's judgment to add as data edits. The panel was never seen rendered by the agent, only through its model's tests, so layout polish may come back as feedback.

Lessons. The prototype milestone paid for itself twice: reading the skill text showed that the four 「8枚以上」 triggers mean "while holding 8", not "acquiring one of", which the old model had wrong without any score showing it (the cap of 4 hid it), and it found the one P-item whose filter the old prefix match silently ignored. An exact-equivalence test between the old and the new model, kept green on every run until the old model was deleted, made a rewrite of how every number is counted a mechanical change instead of a leap. Two of the plan's own instructions had to be bent once the code was in front of us, and both bends were ordering or framing, not design: the deletions could not come before the panel that replaced their last reader (C16), and "compare committed with fresh card data" would have been a check that can never fail (C20) — a stability check has to pin what was published, not what it was computed from. Finally, a position in an id is not a family: the slot after `p_card_search-` holds three, so "new value in a known position takes the family default" needed the exception C12 before it was safe.
...
- 2026-09-22 (close-out): Completion banner, final Progress entry, Outcomes & Retrospective. Reason: the close-out ritual; this is the last edit this plan receives.
