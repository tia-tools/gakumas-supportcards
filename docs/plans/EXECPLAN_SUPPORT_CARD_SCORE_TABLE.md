...
- Decision (D31): GkmasObjectManager is used only to fetch and decrypt the manifest and to download de-obfuscated bundles (`convert_image=False`); decoding, resizing and encoding are ours. It lives as a source tarball pinned to a commit in the gitignored `scripts/images/vendor/` (it is not a package; see Surprises), with its requirements listed in `scripts/images/pyproject.toml`. `decode.py` tries candidate Unity versions (`UNITY_VERSIONS`, newest first) and accepts the first Texture2D whose dimensions are plausible (1–8192 px); a bundle no candidate decodes fails the run by name. Thumbnails are 192 × 108 (16:9, twice the table's 96 × 56 cell), opaque RGB, webp quality 80.
  ...
  Date/Author: 2026-09-20 / agent, from the Milestone 4 probes; not yet confirmed by the user.
...
- Decision (D32): Only art for cards present in `data/cards.generated.ts` is uploaded. `upload.py` holds back every other extracted image and prints which ones.
  ...
  Date/Author: 2026-09-20 / agent; not yet confirmed by the user.
...
Milestone 4 (status 2026-09-20: everything short of the deployment is built and verified locally; the deployment itself needs the user's Cloudflare login) is accepted when `https://gakumas-supportcards.tia.run/` serves the page and `https://gakumas-supportcards.tia.run/img/img_general_csprt-3-0016_full.webp` returns HTTP 200 with `content-type: image/webp` and an immutable cache header.
...
Image pipeline (Milestone 4). `scripts/images/thumbnail.py`: `OBJECT_PATTERN`, `key_for(object_name) -> str | None` (`img_general_csprt-3-0016_full.unity3d` → `img_general_csprt-3-0016_full.webp`), `to_thumbnail(image) -> bytes`. `decode.py`: `UNITY_VERSIONS`, `is_plausible(width, height)`, `decode_texture(raw, versions, load) -> Image` raising `DecodeError` that names every attempt; `load(raw, version)` is injectable, so the rule is tested without UnityPy or a real bundle. `uploads.py`: `plan_uploads(keys, exists) -> UploadPlan(missing, present)`, `card_keys(cards_source) -> set[str]`, `publishable(keys, allowed) -> (keep, held)`. `extract.py` and `upload.py` are the only files doing I/O. `infra/worker/img.ts`: `imageKey(pathname) -> string | null`, `serveImage(request, bucket) -> Response` over an `ImageBucket` (`head`, `get`) that is the slice of R2's API the handler uses, so tests pass a fake and the Workers type package is not needed; `worker.ts` routes `/img/*` to it and everything else to `env.ASSETS`. `vite.config.ts` reuses `imageKey` in a dev-only plugin that serves `scripts/images/out/webp`.
