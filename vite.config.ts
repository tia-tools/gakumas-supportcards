 ...
 * (scripts/images/out/webp, gitignored) so the table shows real card art before and without a
 * deployment. In production the same paths are served from R2 by the Worker (docs/adr/0003);
 * both use the Worker's own `imageKey` rule for what counts as a library image.
 */
...
export default defineConfig({
  plugins: [preact(), tailwindcss(), localImageLibrary(join(import.meta.dirname, "scripts/images/out/webp"))],
  build: {
